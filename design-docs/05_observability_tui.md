...
### Core Engine
-   **Live tab**: Reads BPF maps once per tick (default 1000ms). Computes PPS delta with **EMA smoothing** (α=0.3) on both global and per-ASN sparklines to reduce jitter. Ranks top attackers by drop count and reads exact per-port drop counts from the `port_drop_counts` BPF map. Tracks fetch latency and data freshness.
...
1.  **Live**: Interface status, velocity-colored per-ASN PPS sparklines (Red=rising, Yellow=steady, Green=falling), active swarm table (with iceberg truncation indicator), target heatmap (BPF-backed exact counts), surgical preview + instrumentation footer.
...
3.  **Lists**: Side-by-side whitelist/blacklist editor, CIDR input modal with daemon-matching validation.
...
A three-segment indicator in the persistent HUD header, visible on all tabs:
...
### DogStatsD Export
Optional `--statsd-addr host:port` flag enables UDP export of 8 metrics per tick. Hand-rolled StatsD protocol (no external crate). Non-blocking `send_to` — if the collector is down, sends silently fail with no impact on the 1s tick budget.
